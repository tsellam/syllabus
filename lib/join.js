    ...
    function genStats(opts) {
      ...
      return {
        fillfactor: randItem(opts.fillfactors),
        desize: randItem(opts.desizes),
        tupsize: tupsize,
        pagesize: tupsize * randItem(opts.tupsperpage)
      }
    }
    function log(base, y) {
    ...
    function accessCosts(t, stats) {
      t.card = Math.floor(t.npages * Math.floor((stats.fillfactor*stats.pagesize)) / stats.tupsize);
